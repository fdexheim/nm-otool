...
/*   Updated: 2018/10/04 15:37:43 by fdexheim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		struct fat_arch_64 *arch64, const uint32_t iter)
{
	...
	while (i < iter)
	{
		if (pui32(arch64->cputype, fat_ctrl.m_num) == CPU_TYPE_X86_64)
			env->flags.rightcputype_found = true;
		if (pui32(arch64->cpusubtype, fat_ctrl.m_num) == CPU_SUBTYPE_X86_64_ALL)
			env->flags.rightcpusubtype_found = true;
		...
		if (macho_ctrl.obj_offset > fat_ctrl.obj_size
			|| macho_ctrl.obj_offset + macho_ctrl.obj_size > fat_ctrl.obj_size
			|| !check_arch_bis(env, macho_ctrl))
			return (false);
		arch64++;
		i++;
	}
		...
		struct fat_arch *arch, const uint32_t iter)
{
	...
	while (i < iter)
	{
		if (pui32(arch->cputype, fat_ctrl.m_num) == CPU_TYPE_X86_64)
			env->flags.rightcputype_found = true;
		if (pui32(arch->cpusubtype, fat_ctrl.m_num) == CPU_SUBTYPE_X86_64_ALL)
			env->flags.rightcpusubtype_found = true;
