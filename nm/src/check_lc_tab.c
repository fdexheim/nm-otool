...
/*   Updated: 2018/10/02 15:37:40 by fdexheim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/nm.h"

static bool				check_tab_size_to_section_32(const size_t sizeofcmds,
...
static bool				check_tab_size_to_section_64(const size_t sizeofcmds,
		t_obj_ctrl macho_ctrl)
{
		...
		struct load_command *cmds_start, t_obj_ctrl macho_ctrl)
{
	...
	if (scmds_control != pui32(hdr->sizeofcmds, macho_ctrl.m_num))
		return (false);
	return (check_tab_size_to_section_32(
				pui32(hdr->sizeofcmds, macho_ctrl.m_num), macho_ctrl));
}
		...
		struct load_command *cmds_start, t_obj_ctrl macho_ctrl)
{
	...
	if (scmds_control != pui32(hdr64->sizeofcmds, macho_ctrl.m_num))
		return (false);
	return (check_tab_size_to_section_64(
				pui32(hdr64->sizeofcmds, macho_ctrl.m_num), macho_ctrl));
}
